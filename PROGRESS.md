...
|---------|------|-------|---------|
| 30 | May 2026 | R3F full migration (Phase 2) | Eliminated CSS canvas-content hybrid; CameraSync removed → SceneSetup (camera restore + LOD/zoom, no CSS writing); fragments + clusters render as drei Html in R3F scene at y-flipped canvas positions; ConnectorLayer rewritten as Three.js Line geometry (drei Line, lineWidth, dashed, additive blending for strong glow); Connector.tsx rewritten as R3F component using getBezierPoints3D(); Cluster.tsx + SeedFragment.tsx CSS positioning stripped; canvas.css canvas-content removed; selection rect as screen-space absolute div outside R3F (canvasToScreen helper); CanvasCommandMenu as Html in R3F scene; Z_LAYERS + getFragmentZ() for subtle depth parallax; one coordinate system — no hybrid |
| 29 | May 2026 | R3F canvas migration (Phase 1) | Replaced CSS transform pan-zoom with React Three Fiber orthographic camera; installed three + @react-three/fiber + @react-three/drei + @types/three; usePanZoom.ts rewritten to control OrthographicCamera imperatively (cameraRef + sizeRef + wrapperRef); CanvasBackground.tsx rewritten as R3F ShaderMaterial plane (GLSL dot grid, same offset formula as old SVG version); Canvas.tsx restructured: R3F Canvas (pointer-events:none) renders background + CameraSync component; CameraSync useFrame drives canvas-content CSS transform imperatively (no React re-renders), updates LOD state on threshold crossings, throttles displayZoom state, persists viewport ~1s debounced; hybrid approach chosen: fragments/clusters/connectors stay in CSS-transformed div (not Html portals) because src/edges/ was off-limits; navigateCamera() replaces setTransform() for animated pan (CSS transition + camera update); toCanvas() uses cameraRef/sizeRef for screen→canvas-space conversion; build passes clean, zero console errors, fragment cards interactive, dot grid rendering |
| 28 | May 2026 | Cross-exploration links (prestige mechanic) | CrossLink type in types.ts; webs_cross_links localStorage store with getCrossLinks/addCrossLink/updateCrossLinkLabel/getCrossLinksForExploration in crossLinks.ts; validateCrossLink() AI function in generate.ts (same pattern as validateConnectionLabel); CrossLinkModal.tsx 3-step modal (choose exploration → choose fragment → confirm) rendered via createPortal in Canvas.tsx; "Link to another exploration →" menu item in FragmentMenu (only visible at depthScore ≥ 200); depthScore derived from userConnectionsList in Canvas.tsx and threaded down to Fragment → FragmentCard → FragmentMenu; ⟷ N badge in LibraryPanel with tooltip (title attr) listing linked exploration names; LINKED badge in StatusBar when exploration has cross-links; first-ever cross-link fires lime-green overlay flash + sets webs_first_cross_link_shown localStorage flag; background AI label generation on confirm; projects prop threaded from App.tsx → Canvas.tsx → CrossLinkModal; cross-link.css created; webs-cross-links-changed CustomEvent dispatched on any cross-link mutation |
...
| R3F canvas (Phase 1 — hybrid) | DONE | three + @react-three/fiber + @react-three/drei installed; orthographic camera replaces CSS transform; CanvasBackground is GLSL shader plane; CameraSync drove content div imperatively via useFrame; hybrid: HTML content stayed in CSS div | Session 29 |
| R3F canvas (Phase 2 — full migration) | DONE | CSS canvas-content div eliminated; CameraSync removed; SceneSetup replaces it (no CSS writing, just camera restore + LOD/zoom tracking); all fragments/clusters render as drei Html inside the R3F scene at correct canvas positions (y-flipped for Three.js); ConnectorLayer rewritten as Three.js Line geometry (drei Line component, lineWidth, dashed, additive blending for strong glow); Connector.tsx rewritten as R3F component with getBezierPoints3D bezier sampling; Cluster.tsx + SeedFragment.tsx CSS positioning removed (positioning now from Three.js group); canvas.css canvas-content styles removed; selection rect converted to screen-space overlay outside R3F; CanvasCommandMenu rendered as Html in R3F scene at canvas position; sceneUtils: Z_LAYERS + canvasToScreen helper added to Canvas.tsx; fragment depth variation via getFragmentZ() for subtle parallax; build passes clean; all fragment types + connectors + clusters verified in browser | Session 30 |
| Cross-exploration links | DONE | CrossLink type; webs_cross_links localStorage; getCrossLinks/addCrossLink/getCrossLinksForExploration helpers; validateCrossLink() AI; CrossLinkModal 3-step; depthScore ≥ 200 gate; ⟷ N in LibraryPanel; LINKED in StatusBar; first-link lime flash | Session 28 |
