...
function saveCards(cards) {
  try { localStorage.setItem(STORAGE_KEYS.CARDS, JSON.stringify(cards)); } catch { /* quota */ }
}
...
export default function RememberMode({ newCards }) {
  ...
  function rate(rating) {
    ...
    setCards(prev => {
      const updated = prev.map(c =>
        c.id === card.id ? { ...c, dueDate: dueDate.toISOString(), interval: intervalDays } : c
      );
      saveCards(updated);
      return updated;
    });
  ...
  return (
    <div className="remember-view">
      <div className="card-counter">{idx + 1} / {due.length} due today</div>
      ...
      {flipped && (
        <Space size={12}>
          ...
          <Button
            onClick={() => rate(1)}
            style={{ minWidth: 80, color: 'var(--color-warning)', borderColor: 'rgba(180,83,9,0.3)' }}
          >
            Hard
          </Button>
          <Button
            type="primary"
            onClick={() => rate(2)}
            style={{ minWidth: 80, background: 'var(--color-success)', borderColor: 'var(--color-success)' }}
          >
            Got it
          </Button>
        </Space>
